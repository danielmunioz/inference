...
# from graph_compiler.helpers import get_fns_to_paths_dict
...
def patch_higher_modules(root_module):
    """
    ...
    def rec_reload(module):
        ...
        if not isinstance(module, ModuleType):
            return False
        if first_name(module.__name__) in modules_to_ignore:
            return False
        else:
            pass
        if first_name(module.__name__) in modules_to_update:
            return False
...
def reload_leaves():
    try:
        ...
        importlib.reload(inspect.getmodule(tf.keras.utils.register_keras_serializable))
...
def apply_and_reload(to_reload, to_apply=lambda: None, args=[], kwargs={}, stateful=[]):
    ...
    # currently, the graph compiler gets confused because we are replacing a stateful class.
    # since the graph_compiler module is blacklisted from reloading,
    ...
    try:
        ...
        patch_higher_modules(modules[0])
        # replace classes of existing instances
        replace_classes()
        # get ret
        get_ret(try_getmodule(to_reload))
        return ret
...
def search_var(obj):
    ...
    if "wrapped_for_compiling" in ibdir(ret):
        return ret
...
def replace_classes():
    ...
    for obj in objs:
        ...
        while id(obj.__class__) in class_replacements or first_name(
            fast_getmodule(obj.__class__)
        ) in ["keras"]:
            ...
            try:
                object.__setattr__(
                    obj, "__class__", class_replacements[id(obj.__class__)]
                )
            except:
                break
...
def recursive_reload(module):
    ...
    for attr_name in ibdir(module):
        ...
        if (
            ...
            and first_name(submodule.__name__) not in modules_to_update
        ):
            continue
        if submodule.__name__ == module.__name__:
            continue
        if first_name(submodule.__name__) in modules_to_ignore:
            continue
        if submodule.__name__ in is_relevant:
            ret = True
            continue
    ...
    if module.__name__.split(".")[0] in modules_to_ignore:
        return ret
    if ret:
        is_relevant[module.__name__] = True
        try:
            reload(module)
        except Exception:
            print("failed to reload", module)
...
def is_var_or_instance(module, obj):
    """
    Returns true for instances of custom classes
    originating in ``module`` (not imported from
    somewhere else).
    """
    global not_reloaded_types
    if isinstance(obj, FunctionType):
        return False
    ...
    if isinstance(obj, ModuleType):
        return False
    if hasattr(obj, "__module__") and not obj.__module__ == module.__name__:
        return False
    if obj is __builtins__:
        return False
    not_reloaded_types[type(obj).__name__] = True
    return True
...
def reload(module):
    ...
    global_instances = {
        k: safe_getattr(module, k)
        for k in ibdir(module)
        if is_var_or_instance(module, safe_getattr(module, k))
    }
    ...
    for attr_name in ibdir(module):
        ...
        valid_callable = isinstance(
            attr, (FunctionType, BuiltinFunctionType)
        ) or is_framework_callable(attr)
        if id(attr) in fw_function_replacements and valid_callable:
            try:
