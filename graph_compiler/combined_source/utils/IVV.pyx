...
def process_vmap_fn(graph, fn, args, kwargs):
    ...
    if hasattr(scalar_fn, "wrapped_for_compiling"):
    ...
    graph.vmap_fn = fn
    graph.vmap_node_ids.append(IIX._generate_id())
...
def process_scalar_fn(graph, fn, args, output):
    ...
    subgraph._id_to_function = subgraph_ids
...
def process_vectorized_fn(graph, graph_fn):
    ...
    orig_subgraph._sub_graphs = graph._sub_graphs
...
def generate_vmap_subgraph(
    ...
    from graph_compiler.source_gen import join_args_n_kwargs

    subgraph = graph._sub_graphs[id(f)]
    ...
    if sg.graph.to and sg.graph._to_ivy:
        vmap_fn = "ivy.vmap"
    elif sg.graph.to and not sg.graph._to_ivy:
        if indent == 0 and not sg.vmap_imported and sg.graph.to == "torch":
            fn_header = "import functorch\n" + fn_header
            sg.vmap_imported = True

        if sg.graph.to == "torch":
