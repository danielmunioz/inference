...
class Graph:
    def __init__(
        ...
        **kwargs: Any,
        ...
        self._stateful_param_shapes = [IIX._get_shape(x) for x in self._stateful]
        ...
        self._all_stateful_param_shapes = [IIX._get_shape(x) for x in self._all_stateful]
        ...
        self._fn_signature = dict(inspect.signature(self._fn).parameters) if fn else {}
        ...
        args = to_native(args, nested=True, cont_inplace=True, to_ignore=tvp._to_ignore)
        ...
        self._arg_param_ids = [IIX._get_unique_id(a) for a in arg_parameters]
        ...
        self._arg_param_shapes = [IIX._get_shape(a) for a in arg_parameters]

        # key-word args
        kwargs = to_custom_numpy_type(kwargs, with_numpy=self._with_numpy)
        self._kwargs = dict(**kwargs)
        kwargs = to_native(
            kwargs, nested=True, cont_inplace=False, to_ignore=tvp._to_ignore
        )
        ...
        self._kwarg_param_ids = [IIX._get_unique_id(v) for v in kwarg_parameters]
        ...
        self._kwarg_param_shapes = [IIX._get_shape(v) for v in kwarg_parameters]
    ...
    def _register_output(self, ret: Tuple):
        ...
        self._output_tracked_idxs = ivy.nested_argwhere(
            ...
            or IIX._is_tracked_variable(x),
            to_ignore=tvp._to_ignore,
        )
        self._output_param_ids = [
            IIX._get_unique_id(to_native(x, to_ignore=tvp._to_ignore))
        ...
        for i, id_ in enumerate(self._output_param_ids):
            if id_ in self._arg_param_ids + self._kwarg_param_ids:
                ...
                if is_frontend_array(arg):
                    arg = arg.ivy_array
                from_tracked_var = True if IIX._is_tracked_variable(arg) else False
                ...
                self._output_param_ids[i] = IIX._get_unique_id(ret)
    ...
    def _chain_functions(self, terminal_ids: List):
        ...
        """
        dict_key = IIX._terminal_ids_to_key(terminal_ids)
    ...
    def __call__(self, *args, **kwargs):
        ...
        """
        temp_backend = ivy.current_backend_str()
        temp_backend = self.backend if temp_backend == "" else temp_backend
        if self.to and self.to != "ivy" and self.to != temp_backend:
            ivy.set_backend(self.to)
        if self._to_ivy:
        ...
        if self._transpiling or self._to_ivy:
            args = ivy.nested_map(
                args,
                array_to_new_backend,
                include_derived=True,
            ...
            if self._transpiling:
                args = to_native(
                    args, nested=True, cont_inplace=True, to_ignore=tvp._to_ignore
                )
                kwargs = to_native(
                    kwargs, nested=True, cont_inplace=True, to_ignore=tvp._to_ignore
                )
        else:
        ...
        self._output = self._scripted_call(*args, **kwargs, **self.constants)
        if ivy.current_backend_str() != temp_backend:
            ivy.set_backend(temp_backend)
        return self._output
    ...
    def list_missing_frontends(
        ...
        """
        frequency = IIX._find_missing_frontends(self)
        msg = IIX._format_missing_frontends_msg(frequency)
        if save_json:
            with open(save_json, "w") as fp:
                data = {freq[0]: {"count": freq[1]} for freq in frequency}
                json.dump(data, fp, indent=4)
