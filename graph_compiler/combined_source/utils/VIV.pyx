...
import IIX 
import VII as glob
from . VVV import FUNC_TO_PATH
from . IVV import generate_vmap_subgraph
...
class SourceGenerator:
    ...
    def generate_fn_output(self, graph=None) -> str:
        ...
        subclass_values = ivy.multi_index_nest(output_reprs, subclass_idx)
        ...
        output_reprs = nest_to_str(output_reprs, subclass_idx, subclass_names)
    ...
    def generate_inner_fn_args(self, f: Callable, from_: str) -> str:
        ...
        _idxs = f.with_tracked_slices + subclass_idx
        _inits = ["slice" for idx in f.with_tracked_slices] + subclass_names
        args_reprs = nest_to_str(args_reprs, _idxs, _inits)
        return args_reprs
    ...
    def generate_inner_fn_output(self, f: Callable) -> str:
        ...
        reprs = []
        flat_reprs = IIX.flatten(output_reprs)
        for o in flat_reprs:
...
def nest_to_str(nest, _idxs=None, _inits=None, _base=True, _index=None):
    ...
    _idxs = ivy.default(_idxs, [])
    _inits = ivy.default(_inits, [])
    _index = [] if _base else _index
    is_subclass_init = _index in _idxs
    if isinstance(nest, (tuple, list)):
        if isinstance(nest, list):
            ...
            opening, closing = "(", ")"
        _args = [
            nest_to_str(item, _idxs, _inits, False, _index + [i])
        ...
        _args = {
            k: nest_to_str(v, _idxs, _inits, False, _index + [k])
    ...
    if _base:
        opening, closing = "", ""
    if is_subclass_init:
        i = _idxs.index(_index)
        opening, closing = f"{_inits[i]}(", ")"
...
def get_fn_name(fn: Callable, to_ivy: bool = False) -> str:
    ...
    if to_ivy:
        return (
            f"ivy.{fn.backend_fn.__qualname__}"
            if not fn.from_tracked_var
            else f"{fn.__name__}"
        )
    if (
        fn.inplace_fn
