...
from graph_compiler.numpy_proxy import custom_np_classes
...
def _record_parameters_info(
    ...
    ids = [
        _get_unique_id(ivy.to_native(p, to_ignore=tvp._to_ignore))
        if to_ivy
        else _get_unique_id(p)
        for p in parameters
    ]
...
def _wrap_function_for_op_logging(
    ...
    def _tracing_function(*args, **kwargs):
        ...
        if graph._with_numpy:
            ...
            # check for function
            if hasattr(fn, "__module__") and fn.__module__ is not None:
        ...
        args = ivy.map_nest_at_indices(
            args,
            node.arg_tracked_idxs,
            lambda x: _delete_parameter(x, graph),
        )
        kwargs = ivy.map_nest_at_indices(
            kwargs,
            node.kwarg_tracked_idxs,
        ...
        output_tracked_var_idxs = ivy.nested_argwhere(
            ret_list, _is_tracked_variable, to_ignore=tvp._to_ignore
        )
        ...
        if fn.__name__ in ["__getattr__", "__getattribute__"]:
            ...
            ret_list = ivy.map_nest_at_indices(
                ret_list, node.output_tracked_idxs, lambda x: _clone_param(x, graph)
            )
        ...
        ret_placeholder = ivy.set_nest_at_indices(
            ret_list,
            node.output_tracked_idxs,
            lambda x_: _delete_parameter(x_, graph),
            shallow=False,
        )
...
def _unwrap_functions_from_op_logging(
    ...
    for _, v in wrapped_dict.items():
        if hasattr(v[1], "wrapped_for_compiling"):
