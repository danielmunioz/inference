...
from graph_compiler.exchange import wrap_backend_fn

from tracing_caching.cache_db import CacheDB
from tracing_caching.tracer import (detect_differences, parse_all_lines,
...
class Cacher():
    ...
    def store_cache(self, traced_data, graph):
        ...
        functions = {f.backend_fn: self._get_func_attr(f) for f in graph._functions}
        functions = self._serialize_object(functions)

        # store cache
        self.db.insert_cache(
            code_loc, code_line, func_def, args_str, kwargs_str, compile_kwargs_str, source_code_str, graph_fn_str, constants, functions
        )
    ...
    def get_matching_graph(self, traced_data, cached_data):
        ...
        if not self.verify_execution_changed(traced_code_src, cached_code_src):
            ...
            functions = self._deserialize_object(
                cached_data.get("functions"))
            functions = [wrap_backend_fn(k, v) for k, v in functions.items()]
            ...
            cached_graph._functions = functions
            return cached_graph
