...
def test_function(func_class, data, *constructor_args, backends=IVY_BACKENDS):
  ...
  for backend in backends:
    ...
    assert ivy.array_equal(output, converted_output), \
      f"Outputs do not match for function {model.__name__} and backend {backend}"


class SimpleNeuralNetwork:
  def __init__(self, num_inputs, num_hidden, num_outputs):
    self.weights_input_to_hidden = ivy.random.random_normal(mean=0, std=1, shape=(num_inputs, num_hidden))
    self.biases_input_to_hidden = ivy.zeros(num_hidden)
    self.weights_hidden_to_output = ivy.random.random_normal(mean=0, std=1, shape=(num_hidden, num_outputs))
    self.biases_hidden_to_output = ivy.zeros(num_outputs)

  def forward(self, inputs):
    hidden_inputs = ivy.matmul(inputs, self.weights_input_to_hidden) + self.biases_input_to_hidden
    ...
    while i < hidden_inputs.shape[0]:
      if any(hidden_inputs[i]) > 0:
        ...
        hidden_outputs[i] = 0
      i += 1
    output = ivy.matmul(hidden_outputs, self.weights_hidden_to_output) + self.biases_hidden_to_output
    return output
...
class DynamicCNN(ivy.Module):
  def __init__(self, num_channels, num_classes):
    super().__init__()
    self.conv1 = ivy.Conv2D(num_channels, 32, [3,3], 1, 'VALID', data_format="NCHW")
