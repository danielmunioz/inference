...
class BaseNetwork(ivy.Module):
    ...
    def __init__(self,
        ...
        ivy.Module.__init__(self, v=v, device=self._spec.dev_strs,
...
class PreNorm(ivy.Module):
    def __init__(self, dim, fn, context_dim=None, epsilon=None, device=None, v=None):
        self._fn = fn
        self._norm = ivy.LayerNorm([dim], eps=epsilon, device=device, new_std=1)
        if isinstance(context_dim, int):
            context_dim = [context_dim]
        self._norm_context = ivy.LayerNorm(context_dim, eps=epsilon, device=device, new_std=1) if \
...
class PerceiverBase(ivy.Module):
    ...
    def _create_variables(self, device, dtype=None):
        latents = _variable(
            ivy.random_uniform(shape=(self._spec.num_latents, self._spec.latent_dim), dtype=dtype, device=device))
        # ToDo: set the correct initializatin scheme for the query here
        decoder_queries = _variable(ivy.random_uniform(shape=self._spec.query_shape + [self._spec.queries_dim], dtype=dtype, device=device))\
    ...
    def _forward(self, data, mask=None, queries=None):
        ...
        if self._fourier_encode_input:
            axis_pos = list(map(lambda size: ivy.linspace(-1., 1., size, dtype=self._dtype, device=self._spec.device), data_shape))
...
class PerceiverIO(PerceiverBase, BaseNetwork):

    def __init__(self, spec: PerceiverIOSpec, v: ivy.Container = None):
        PerceiverBase.__init__(self, spec, v)
        BaseNetwork.__init__(self, spec, v)

    def _forward(self, data, mask=None, queries=None):
        if data.shape[1] == 1:
            data = ivy.squeeze(data, 1)
        return PerceiverBase._forward(self, data, mask, queries)
